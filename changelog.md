...
## [0.0.504] — 2026-09-13 — T3.12 CONCLUÍDA (teste humano executado)

- 2026-09-13 · [Deni.Ai] · Teste humano da T3.12 executado a pedido da CEO, com a ficha real da Felicidade Collective: motor gerou 18 obrigações (ciclo completo de contas a pagar semanal, faturamento dias 10/25, conciliação semanal); baixa em 1 toque (200); baixa em lote 3/3; bloqueio com motivo (200) e baixa em bloqueada rejeitada (400); exceção E10 resolvida pela baixa; dedup provado (2ª execução 0/18); regressão ok. **Bug encontrado e corrigido no teste**: auditoria silenciosamente falhando — coleção auditoria só aceitava acao create/update (migration 0010) e o try/catch engolia o erro; fix na migration 0164 (valores motor_executado/baixa/baixa_lote/bloqueio/delete) + registro_id não vazio; re-teste confirmou registro. Lição registrada: try/catch de auditoria deve logar, não engolir. Estado final preservado para uso real: 13 obrigações (12 previstas + 1 bloqueada), 4 exceções (3 abertas + 1 resolvida). Evidência: evidencias/spec-3-012/ca-3-036-042-teste-humano.md.
